...
import {
  remote, refreshHosts, connectHost, openRecent, recentOf, togglePin, forgetItem, setHostState, setHostExpanded, setPaneOpen,
  type RemoteHost,
} from '../../model/remote';
...
function toggleHost(pane: Pane, h: RemoteHost): void {
  ...
  if (recentOf(h.name).length === 0) return;
...
function onRecentContextMenu(e: MouseEvent, h: RemoteHost, path: string, pinned: boolean): void {
  e.preventDefault();
  openContextMenu(e.clientX, e.clientY, [
    pinned
      ? { label: '고정 해제', run: () => void togglePin(h.name, path) }
      : { label: '상단에 고정 (P)', key: 'p', run: () => void togglePin(h.name, path) },
    { label: '최근 목록에서 제거', run: () => void forgetItem(h.name, path) },
  ]);
}
        ...
        <template v-for="h in pane === 'favorite' ? remote.favorites : remote.all" :key="h.name">
          ...
          >
            <span
              ...
              :class="recentOf(h.name).length ? (isExpanded(h) ? 'codicon-chevron-down' : 'codicon-chevron-right') : ''"
            />
          ...
          <!-- 최근 연 폴더 (VS Code Remote Explorer 의 호스트 하위 항목) — 이름 + 흐린 전체 경로 한 줄.
               클릭은 선택, 접속은 호스트 행과 같은 hover 액션 (→ 현재 탭 대체 / 새 탭). 상단 고정(pin)은
               우클릭 토글 — 아이콘만 pinned 로 바뀌고 자리는 relay 가 위로 -->
          <template v-if="isExpanded(h)">
            <div
              v-for="{ path: p, pinned } in recentOf(h.name)"
              :key="p"
              class="row recent"
              :class="{ selected: selected === recentKey(pane, h, p) }"
              :title="p"
              @click.stop="selected = recentKey(pane, h, p)"
              @contextmenu="onRecentContextMenu($event, h, p, pinned)"
            >
              <span class="codicon type-icon" :class="pinned ? 'codicon-pinned' : 'codicon-folder'" />
...
/* VS Code 트리 행 — 22px 한 줄, 들여쓰기·트위스티·아이콘 (2026-09-11 카드형에서 되돌림) */
.row {
  display: flex;
  align-items: center;
  height: 22px;
